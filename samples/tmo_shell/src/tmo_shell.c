...
int tcp_create_tls_core(const struct shell *shell, size_t argc, char **argv, int family)
{
	...
	ret = setsockopt(sd, SOL_TLS, TLS_SEC_TAG_LIST, sec_tag_list, sizeof(sec_tag_list));
	if (ret < 0) {
		shell_error(shell, "Failed to set TLS_SEC_TAG_LIST option: %d", errno);
		ret = -errno;
	}

	if (strlen(TLS_PEER_HOSTNAME)) {
		ret = setsockopt(sd, SOL_TLS, TLS_HOSTNAME, TLS_PEER_HOSTNAME,
				 sizeof(TLS_PEER_HOSTNAME));
		if (ret < 0) {
			shell_error(shell, "Failed to set TLS_HOSTNAME option: %d", errno);
			ret = -errno;
		}
	}
...
int udp_create_dtls_core(const struct shell *shell, size_t argc, char **argv, int family)
{
	...
	if (!key_credential_added) {
		ret = tls_credential_add(CLIENT_KEY_TAG, TLS_CREDENTIAL_PRIVATE_KEY,
					 dev_private_key, sizeof(dev_private_key));
		if (ret < 0) {
			shell_error(shell, "<<< Failed to register dev private key  %d >>>\n", ret);
		} else {
			shell_info(shell,
				   "===== Added DTLS dev private key credential! =========\n");
...
int tmo_dtls_cred(const struct shell *shell, char *cred_type, char *operation, int sd)
{
	...
	strcat(filename, ".pem");
	if (((strcmp(operation, "w") != 0) && (strcmp(operation, "d") != 0)) || sd < 0 ||
	    sd >= MAX_SOCK_REC) {
	...
	if (strcmp(operation, "w") == 0) {
		if ((ret = tmo_set_modem(STORE_CERT, (union params_cmd *)u, sd)) == 0) {
			...
			shell_error(shell, "Failed to store %s - sd:%d errno: %d", cred_type, sd,
				    errno);
		}
	} else {
		if ((ret = tmo_set_modem(DEL_CERT, (union params_cmd *)filename, sd)) == 0) {
			...
			shell_error(shell, "Failed to delete %s using sd:%d  error:%d", cred_type,
				    sd, errno);
		}
	}
	return ret;
...
int tmo_profile_dtls(const struct shell *shell, char *operation, int sd)
{
	...
	if (((strcmp(operation, "a") != 0) && (strcmp(operation, "d") != 0)) || sd < 0 ||
	    sd >= MAX_SOCK_REC) {
		shell_error(shell, "\n Invalid Inputs, operation %s sd %d ", operation, sd);
	}
	...
	if (strcmp(operation, "a") == 0) {
		if (tmo_set_modem(CREATE_CERT_PROFILE, (union params_cmd *)u, sd) == 0) {
			...
			shell_error(shell, "Failed to create cert profile, errno: %d sd %d", errno,
				    sd);
			ret = -errno;
			return ret;
		}
		...
		ret = setsockopt(sd, SOL_TLS, TLS_MURATA_USE_PROFILE, &profile_id, sizeof(int));
		client_verify = 1;
		ret = setsockopt(sd, SOL_TLS, TLS_MURATA_CLIENT_VERIFY, &client_verify,
				 sizeof(int));
		peer_verify = 0;
		ret = setsockopt(sd, SOL_TLS, TLS_PEER_VERIFY, &peer_verify, sizeof(int));
		if (ret < 0) {
		...
		if (tmo_set_modem(DELETE_CERT_PROFILE, (union params_cmd *)u, sd) == 0) {
			...
			shell_error(shell, "Failed to delete cert profile: errno %d sd %d", errno,
				    sd);
			ret = -errno;
		}
	}
...
int sock_connect(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!(socks[sock_idx].flags & sock_cmd_parent_bm)) {
		shell_warn(shell, "Warning: Socket %d is a %s socket", sd,
	...
	if (socks[sock_idx].flags & (BIT(sock_tls) | BIT(sock_dtls))) {
		...
		ret = setsockopt(sd, SOL_TLS, TLS_PEER_VERIFY, &peer_verify_val,
				 sizeof(peer_verify_val));
	}
	...
	if (!ret) { // try dns
		...
		if (socks[sock_idx].flags & BIT(sock_open)) {
			...
			if (strncmp(iface->if_dev->dev->name, "murata", 6) == 0) {
				...
				shell_error(shell, "Unknown interface: %s",
		...
		if (socks[sock_idx].flags & (BIT(sock_tls) | BIT(sock_dtls))) {
			ret = setsockopt(sd, SOL_TLS, TLS_HOSTNAME, host, strlen(host));
		}
		...
		if (target.sa_family != ai_family) {
			shell_error(shell, "Socket %d is a %s socket, got %s address", sock_idx,
...
int sock_sendto(const struct shell *shell, size_t argc, char **argv)
{
	...
	int stat = zsock_sendto(sd, argv[4], strlen(argv[4]), 0, &target,
				ai_family == AF_INET ? sizeof(struct sockaddr_in)
...
int sock_sendb(const struct shell *shell, size_t argc, char **argv)
{
	...
	while (total < sendsize || sendsize == 0) {
		...
		if (stat == -1) {
			if (errno == EMSGSIZE) {
				shell_warn(shell,
					   "Note: EMSGSIZE (errno=%d) may be cause by a fragment "
					   "being larger than network MTU.",
					   EMSGSIZE);
			}
			shell_error(shell, "send failed, errno = %d", errno);
			break;
...
#define MARGIN 5 // print partial left/right adjustment
	...
	for (int i = 0; i < len; i++) {
		if (buf[i] != (0x20 + (i % 97))) {
			...
			shell_error(shell,
				    "buffer mismatch: buf[%d] = 0x%x, should be 0x%x, buf from "
				    "[%d]: %s",
...
int sock_recvb(const struct shell *shell, size_t argc, char **argv)
{
	...
	while (total < recvsize || recvsize == 0) {
		stat = zsock_recv(sd, mxfer_buf + total, MIN(recvsize - total, max_fragment),
				  ZSOCK_MSG_DONTWAIT);
		if (stat == -1) {
...
int sock_rcvfrom(const struct shell *shell, size_t argc, char **argv)
{
	...
	int ai_family = (socks[sock_idx].flags & BIT(sock_v6)) ? AF_INET6 : AF_INET;
	addrLen =
		(ai_family == AF_INET6) ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
	memset(mxfer_buf, 0, XFER_SIZE);
	char addrbuf[NET_IPV6_ADDR_LEN];
	stat = zsock_recvfrom(sd, mxfer_buf, XFER_SIZE, ZSOCK_MSG_DONTWAIT,
			      (struct sockaddr *)&target, &addrLen);
#if IS_ENABLED(CONFIG_NET_IPV6)
	void *addr = (ai_family == AF_INET6) ? (void *)&net_sin6(&target)->sin6_addr
					     : (void *)&net_sin(&target)->sin_addr;
	uint16_t port = ntohs((ai_family == AF_INET6) ? net_sin6(&target)->sin6_port
						      : net_sin(&target)->sin_port);
#else
...
int sock_close(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!(socks[sock_idx].flags & sock_cmd_parent_bm)) {
		shell_warn(
			shell, "Warning: Socket %d is a %s socket", sd,
			(socks[sock_idx].flags & (BIT(sock_udp) | BIT(sock_dtls)) ? "UDP" : "TCP"));
	}
...
int cmd_list_socks(const struct shell *shell, size_t argc, char **argv)
{
	...
	for (int i = 0; i < MAX_SOCK_REC; i++) {
		...
		if (socks[i].flags & BIT(sock_open)) {
			...
			shell_print(shell, "%d: iface=%s proto=%s %s%s%s", socks[i].sd,
				    iface->if_dev->dev->name, protos[proto_x],
...
void shell_help_modem(const struct shell *shell)
{
	shell_print(shell, "tmo modem <iface> <cmd_str>\n"
			   "                  <cmd_str>: apn | awake | conn_sts | edrx | golden | "
			   "iccid | imei | imsi |\n"
			   "                             ip | ip6 | msisdn | psm | ptw | sim | "
			   "sleep | ssi | version | wake");
}
...
int cmd_modem(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!strstr(iface->if_dev->dev->name, "murata")) {
		shell_error(shell, "dev - %s Not Supported; only Murata 1SC is supported",
			    iface->if_dev->dev->name);
		return -EINVAL;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	tmo_tcp_sub, SHELL_CMD(close, NULL, "<socket>", tcp_close),
...
void dump_addrinfo(const struct shell *shell, const struct addrinfo *ai)
{
	...
	if (ai->ai_family == AF_INET6) {
		zsock_inet_ntop(ai->ai_family, &net_sin6(ai->ai_addr)->sin6_addr, mxfer,
				sizeof(mxfer));
	} else {
		zsock_inet_ntop(ai->ai_family, &net_sin(ai->ai_addr)->sin_addr, mxfer,
				sizeof(mxfer));
	}
	shell_print(shell,
...
int cmd_dnslookup(const struct shell *shell, size_t argc, char **argv)
{
	...
	while (ai) {
		...
		if (ai->ai_family == AF_INET6) {
			zsock_inet_ntop(ai->ai_family, &net_sin6(ai->ai_addr)->sin6_addr, mxfer_buf,
					128);
		} else {
			zsock_inet_ntop(ai->ai_family, &net_sin(ai->ai_addr)->sin_addr, mxfer_buf,
					128);
		}
...
SHELL_STATIC_SUBCMD_SET_CREATE(tmo_wifi_commands,
			       SHELL_CMD(connect, NULL,
					 ...
					 cmd_wifi_connect),
			       SHELL_CMD(disconnect, NULL, "\"<iface>\"", cmd_wifi_disconnect),
			       SHELL_CMD(mac, NULL, "\"<iface>\"", cmd_wifi_mac),
...
int process_cli_cmd_modem_psm(const struct shell *shell, size_t argc, char **argv, int sd)
{
	...
	if (argc == 8) {
		...
		if ((mode >= 0 && mode <= 1) && (t3412_mul >= 0 && t3412_mul <= 7) &&
			...
			shell_print(shell,
				    "Set PSM mode: %d,\n"
				    "    T3412 Unit: %d, T3412 Value: %d, T3324 Unit: %d, T3324 "
				    "Value: %d",
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD(respond, &ble_smp_9116_respond, "Send response.", NULL), SHELL_SUBCMD_SET_END);
#endif

#if IS_ENABLED(CONFIG_BT_PERIPHERAL)
#include "tmo_ble_demo.h"
SHELL_STATIC_SUBCMD_SET_CREATE(
	ble_adv_sub, SHELL_CMD(conn, NULL, "Advertise as connectable", cmd_ble_adv_conn),
	SHELL_CMD(eddystone, NULL, "Advertise as eddystone beacon", cmd_ble_adv_ebeacon),
	SHELL_CMD(ibeacon, NULL,
...
int cmd_dfu_download(const struct shell *shell, size_t argc, char **argv)
{
	if (argc < 2) {
		shell_error(shell, "Missing required arguments");
		shell_print(shell,
			    "Usage: tmo dfu download <target> [filename]\n"
...
int cmd_battery_voltage(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (status != 0) {
		...
		if (!attached) {
			...
			shell_print(shell, "Battery voltage %d.%03dV", millivolts / 1000,
				    millivolts % 1000);
		}
	}
	return status;
...
int cmd_battery_discharge(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (argc > 2) {
		shell_error(shell, "Incorrect parameters");
		shell_print(shell,
			    "usage: tmo battery discharge [set point (optional, default: 60)]");
		return -1;
	}
	if (argc == 2) {
...
int udp_cred_dtls(const struct shell *shell, size_t argc, char **argv)
{
	if (argc < 3) {
		...
		shell_print(
			shell,
			"Usage: tmo udp %s <operation> <socket>>\n"
...
int udp_profile_dtls(const struct shell *shell, size_t argc, char **argv)
{
	if (argc < 3) {
		...
		shell_print(
			shell,
			"Usage: tmo udp profile <operation> <socket>>\n"
...
int cmd_json_print_settings(const struct shell *shell, size_t argc, char **argv)
{
	...
	printf("Transmit: %s\nInterface %d\nNumber transmissions: %d\nTransmit interval: %d secs\n",
	       ws.transmit_flag ? "ENABLED" : "DISABLED", ws.iface_type, ws.number_http_requests,
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	tmo_json_sub, SHELL_CMD(base_url, NULL, "Set JSON base URL", cmd_json_base_url),
...
SHELL_STATIC_SUBCMD_SET_CREATE(tmo_mfg_sub, SHELL_CMD(all, NULL, "Run all MFG tests", cmd_mfg_test),
...
SHELL_STATIC_SUBCMD_SET_CREATE(tmo_test_sub, SHELL_CMD(mfg, &tmo_mfg_sub, "Manufacturing", NULL),
			       SHELL_CMD(qa, NULL, "Quality Assurance (TBD)", cmd_qa_test),
			       SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(
	tmo_dfu_sub, SHELL_CMD(auth_key, NULL, "Set FW download auth key", cmd_dfu_auth_key),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	tmo_battery_sub, SHELL_CMD(charger, NULL, "Get charger status", cmd_charging_status),
...
SHELL_STATIC_SUBCMD_SET_CREATE(tmo_bootloader_sub,
			       SHELL_CMD(current, NULL, "Get current active slot",
...
int cmd_tmo_cert_modem_load(const struct shell *shell, int argc, char **argv)
{
	...
	cparams.cert = &cert;
	int sock =
		zsock_socket_ext(AF_INET, SOCK_STREAM, IPPROTO_TCP, net_if_get_by_index(MODEM_ID));
	if (sock == -1) {
...
#if CONFIG_MODEM
			       SHELL_CMD(modem_load, NULL, "Send loaded certificate to modem",
					 cmd_tmo_cert_modem_load),
#endif
			       SHELL_SUBCMD_SET_END);
#endif
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_pm,
	SHELL_CMD_ARG(get, &dsub_device_name, "Get a device's power management state", cmd_pmget, 2,
		      2),
	SHELL_CMD_ARG(lock, &dsub_device_name, "Lock a devices power management state", cmd_pmlock,
		      2, 2),
	SHELL_CMD_ARG(off, &dsub_device_name, "Put a device into the off/unpowered state",
		      cmd_pmoff, 2, 2),
	SHELL_CMD_ARG(on, &dsub_device_name, "Put a device into the on/powered state", cmd_pmon, 2,
		      2),
	SHELL_CMD_ARG(resume, &dsub_device_name, "Resume a device from the suspended state",
		      cmd_pmresume, 2, 2),
	SHELL_CMD_ARG(suspend, &dsub_device_name, "Put a device into the suspended state",
		      cmd_pmsuspend, 2, 2),
	SHELL_CMD_ARG(unlock, &dsub_device_name, "Unlock a devices power management state",
		      cmd_pmunlock, 2, 2),
	SHELL_SUBCMD_SET_END);
#endif
...
#if CONFIG_PM
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_sys_pm, SHELL_CMD(active, NULL, "Put a device into the active state", cmd_pmsysactive),
...
SHELL_STATIC_SUBCMD_SET_CREATE(tmo_file_sub, SHELL_CMD(cp, NULL, "Copy a file", tmo_cp),
