...
static int pm_waken_devices(void)
{
    ...
    for (const struct device *dev = devs + devc - 1; dev >= devs; dev--)
    {
         ...
         * runtime PM enabled.
         */
        if (pm_device_is_busy(dev) || pm_device_state_is_locked(dev) || /* pm_device_wakeup_is_enabled(dev) || */
        ...
        {
            LOG_INF("%s(): device name: %s, call status: %d", __func__, dev->name, ret);
            LOG_INF("%s(): device name: %s, call status: %s", __func__, dev->name, pm_device_state_str(-ret));
            continue;
        }
        else if (ret < 0)
        {
...
static void pm_resume_devices(void)
{
    ...
    {
        int rc = pm_device_action_run(__pm_device_slots_start[i],
                                      PM_DEVICE_ACTION_RESUME);
        LOG_INF("%s(): device name: %s, device status: %d", __func__, __pm_device_slots_start[i]->name, rc);
        LOG_INF("%s(): device name: %s, call status: %s", __func__,  __pm_device_slots_start[i]->name, pm_device_state_str(-rc));
    }
