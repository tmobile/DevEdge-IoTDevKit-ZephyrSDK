...
#define TLS_SNI_HOSTNAME CONFIG_AWS_URL
#define APP_CA_CERT_TAG 1
#define APP_DEVICE_CERT_TAG 2
...
static int get_mqtt_broker_addrinfo(void)
{
	...
	while (true) {
		...
		hints.ai_protocol = 0;
		rc = zsock_getaddrinfo(CONFIG_AWS_URL, "8883",
					   &hints, &haddr);
		if (rc == 0) {
		...
		LOG_INF("DNS not resolved for %s:%d (ERR: %s), retrying in 15s",
			CONFIG_AWS_URL,
			SERVER_PORT,
			gai_strerror(rc));
...
void main(void)
{
	...
	if(strlen(CONFIG_NET_SAMPLE_SSID)){
		struct wifi_connect_req_params cnx_params;
		cnx_params.channel = WIFI_CHANNEL_ANY;
		cnx_params.ssid = CONFIG_NET_SAMPLE_SSID;
		cnx_params.ssid_length = strlen(CONFIG_NET_SAMPLE_SSID);
		cnx_params.security = strlen(CONFIG_NET_SAMPLE_PSK) ? WIFI_SECURITY_TYPE_PSK : WIFI_SECURITY_TYPE_NONE;
		cnx_params.psk = CONFIG_NET_SAMPLE_PSK;
		cnx_params.psk_length = strlen(CONFIG_NET_SAMPLE_PSK);
		net_mgmt(NET_REQUEST_WIFI_CONNECT, net_if_get_default(),
